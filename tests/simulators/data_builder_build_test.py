...
class MyTestCase(unittest.TestCase):
    ...
    def test_case(self):
        ...
        fb.add_column('segmentation', 'get_category',
                      ...
                      quantity=0.2)
        fb.fbpm.save()
        fb.build_columns(rows=1000, filename='data/comms.csv')
    ...
    def test_transaction(self):
        ...
        fb.fbpm.save()
        fb.build_columns(rows=1000, filename='data/transaction.csv')
    ...
    def test_account(self):
        ...
        fb.add_column('payment_amount', 'get_distribution', method='beta', offset=np.random.randint(30, 300),
                      precision=2, a=5, b=2)
        fb.fbpm.save()
        fb.build_columns(rows=1000, filename='data/account.csv')
    ...
    def test_customer_build(self):
        ...
        for label, offset in label_offset.items():
            fb.add_column(label, 'get_distribution', method='beta', offset=np.random.randint(offset[0], offset[1]),
                          precision=2, a=5, b=2)
        fb.fbpm.save()
        fb.build_columns(rows=101, filename='data/customer.csv')
